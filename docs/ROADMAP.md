...
| **M6** | Production runner (Flink on-K8s or Dataflow) + pipeline monitoring (lag, throughput, failures) — *config-driven runner selection (`direct`/`dataflow`/`flink` alias → `PipelineOptions`, wired through `build_engine`) + throughput counters on the `PipelineResult`; **Dataflow submit path shipped** (SDK worker image, streaming config example, submit docs) and **Flink-on-K8s** self-hosted portable runner (`deploy/flink`: JobManager + TaskManagers with SDK-harness sidecar + Beam job server, RocksDB→S3 checkpoints; `portable` runner alias). **order: Dataflow first (managed), then Flink-on-K8s**. Full monitoring on the `PipelineResult`: throughput (`rows_in`/`records_out`), event lag (`event_lag_ms` distribution), failures (`records_failed`, via a per-bundle guard that drops a poison window instead of killing the stream). Validated on the DirectRunner; live GCP/k3s runs pending* | ✅ runner + monitoring (local) |
| **M7** | KubeVerdict alerting + optional retraining loop back to the datalake — *alerting: `kubeverdict-alert` SPI sink pushes anomalous signals (severity ≥ threshold = the NORMAL→INCIDENT verdicts) to kube-verdict's webhook as they happen, best-effort, stdlib `urllib`; complements the `signal-store` pull path (D7). Retraining loop: `inference/retrain.py` closes the feedback — the KB selects the NORMAL history (INCIDENT stretches excised), PatchTST is retrained on it via `train_reference`, and checkpoints are versioned with a `latest` pointer. **Auto-resume wired (#33)**: the `*-infer` detectors take `checkpoint_dir` and follow that pointer via `resolve_latest`, picking up the newest retrained set on the next run with no config change (the model `spec` travels in the manifest); before the first retrain they fall back to z-score* | ✅ done |
...
The Mimir source (C9a) also has a real-run CLI — `python -m
connectors.sources.mimir_cli` runs one live `query_range` (relative `--lookback`
window or explicit `--start/--end`, tenant, group-by) and prints the aligned
pivot rows as a summary or NDJSON (#33): the read-side smoke test against a live
cluster, mirroring the `connectors.ingest` write CLI.
